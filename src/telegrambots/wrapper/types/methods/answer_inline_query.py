...
class AnswerInlineQuery(TelegramBotsMethodNoOutput):
    # --- description here ---
    ...
    inline_query_id: str = field(
        metadata={"ac_type": [str], "ac_name": "inline_query_id"}
    )
    """Unique identifier for the answered query
    """

    results: list[InlineQueryResult] = field(
        metadata={"ac_type": [InlineQueryResult], "ac_name": "results"}
    )
    """A JSON-serialized array of results for the inline query
    """

    cache_time: int = field(
        default=300, metadata={"ac_type": [int], "ac_name": "cache_time"}
    )
    """The maximum amount of time in seconds that the result of the inline query may be cached on the server. Defaults to 300.
    """

    is_personal: Optional[bool] = field(
        default=None, metadata={"ac_type": [bool], "ac_name": "is_personal"}
    )
    """Pass *True*, if results may be cached on the server side only for the user that sent the query. By default, results may be returned to any user who sends the same query
    """

    next_offset: Optional[str] = field(
        default=None, metadata={"ac_type": [str], "ac_name": "next_offset"}
    )
    """Pass the offset that a client should send in the next query with the same text to receive more results. Pass an empty string if there are no more results or if you don't support pagination. Offset length can't exceed 64 bytes.
    """

    switch_pm_text: Optional[str] = field(
        default=None, metadata={"ac_type": [str], "ac_name": "switch_pm_text"}
    )
    ...
    switch_pm_parameter: Optional[str] = field(
        default=None, metadata={"ac_type": [str], "ac_name": "switch_pm_parameter"}
    )
